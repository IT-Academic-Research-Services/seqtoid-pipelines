...
def parse_inventory(
    ...
    for line_number, raw_line in enumerate(
        ...
        try:
            ...
            raise ReferencePreparationError(
                f"Invalid JSON from s5cmd on line "
                f"{line_number}: {exc}"
            ) from exc

        if not isinstance(payload, dict):
            raise ReferencePreparationError(
                f"Invalid JSON record from s5cmd "
                f"on line {line_number}"
            )

        if payload.get("error"):
            raise ReferencePreparationError(
                f"s5cmd reported an error on line "
        ...
        if "/" in rel:
            raise ReferencePreparationError(
                f"Unexpected nested reference object: {rel}"
            )
        ...
        if not isinstance(size, int) or size < 0:
            raise ReferencePreparationError(
                f"Missing or invalid size for reference "
                f"object: {rel}"
            )
    ...
    if not objects:
        raise ReferencePreparationError(
            f"No reference objects found below "
            f"{prefix.rstrip('/')}"
        )
...
def validate_gpu_inventory(
    ...
    if missing or extra:
        ...
        raise ReferencePreparationError(
            "GPU MMseqs S3 inventory does not match "
            "expected database: "
            + "; ".join(pieces)
        )
...
def build_inventory(
    ...
    if output.returncode != 0:
        raise ReferencePreparationError(
            f"Failed to list reference prefix "
            f"{spec.s3_source}; "
            f"stderr={output.stderr.strip()}"
        )

    objects = parse_inventory(
...
def load_inventory(
    ...
    try:
        ...
        raise ReferencePreparationError(
            f"Unable to read reference inventory "
            f"{path}: {exc}"
        ) from exc
...
def validate_local_inventory(
    ...
    for path in local_dir.iterdir():
        ...
        if not path.is_file():
            raise ReferencePreparationError(
                ...
                f"reference directory: {path.name}"
            )
    ...
    if missing or extra or wrong_size:
        ...
        raise ReferencePreparationError(
            "Local GPU reference does not match "
            "S3 inventory: "
            + "; ".join(problems)
        )
...
def validate_mmseqs(
    ...
    if not db.is_file():
        raise ReferencePreparationError(
            f"MMseqs database base file is missing: {db}"
        )
    ...
    if result.returncode != 0:
        raise ReferencePreparationError(
            f"mmseqs dbtype failed for {db}; "
            f"stderr={result.stderr.strip()}"
        )

    LOG.info(
        "MMseqs GPU database validated: %s",
        db,
    )
...
def chown_cache_root(
    ...
    try:
        ...
        raise ReferencePreparationError(
            f"Unable to set cache-root ownership "
            f"on {path}: {exc}"
        ) from exc
    ...
    try:
        ...
        if (
            ...
            try:
                ...
                LOG.exception(
                    "Failed to restore previous GPU reference cache"
                )
...
def prepare_reference(
    ...
    if backend != "mmseqs-gpu":
        raise ReferencePreparationError(
            f"Unsupported backend for GPU preparer: {backend}"
        )
    ...
    with tempfile.TemporaryDirectory(
        ...
        LOG.info(
            "Canonical GPU MMseqs reference version: %s",
            version,
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare and validate a SeqToID "
            "MMseqs GPU worker reference."
        )
    )
