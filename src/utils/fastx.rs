        ...
        match path2 {
            None => {
                ...
                while let Some(result) = reader.next() {
                    ...
                    let record = result.map_err(|e| anyhow!("Parse error: {}", e))?;
                    ...
                    // Manually construct FASTQ record: @id\nseq\n+\nqual\n
            ...
            Some(path2) => {
                ...
                let mut reader2 = parse_fastx_file(&path2)
                    .map_err(|e| anyhow!("Failed to open FASTQ {}: {}", path2.display(), e))?;
                let mut buffer = Vec::with_capacity(chunk_size);
                let mut read_count: u64 = 0;
                ...
                loop {
                    ...
                    let r1_record = match r1_opt {
                        Some(Ok(rec)) => rec,
                        Some(Err(e)) => return Err(anyhow!("R1 parse error: {}", e)),
                        None => break,
                    };
                    ...
                    let r2_record = match r2_opt {
                        Some(Ok(rec)) => rec,
                        Some(Err(e)) => return Err(anyhow!("R2 parse error: {}", e)),
                        None => return Err(anyhow!("R2 ended before R1 at read {}", read_count + 1)),
                    };
                    ...
                    if !compare_read_ids_bytes(r1_head, r2_head) {
                        return Err(anyhow!("ID mismatch at read {}: {:?} vs {:?}", read_count + 1, r1_head, r2_head));
                    }
