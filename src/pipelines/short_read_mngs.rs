...
use crate::utils::streams::{deinterleave_fastq_stream, join_with_error_handling,
                            ...
                            ParseOutput, ToBytes, monitor_stream, batch_rayon_process, parse_bytes, parse_lines, fanout_to_channels, read_file_lines_batched};
...
///
/// * `config` - RunConfig struct
/// * `m8_path` - line-based m8 format
/// * `summary_path` - hit summary information
...
pub async fn generate_taxon_counts(
        ...
        .map(|(worker_idx, mut rx)| {
            ...
            tokio::spawn(async move {
                ...
                info!(
                    ...
                    ct, worker_idx, batches, total_pairs, max_recv_wait, max_batch_process, start.elapsed()
                );
    ...
    // Producer: read both materialized files and pair lines in lockstep
    let m8_file = TokioFile::open(&m8_path)
        .await
        .map_err(|e| PipelineError::IOError(format!("Failed to open m8 file {}: {}", m8_path.display(), e)))?;
    let summary_file = TokioFile::open(&summary_path)
        .await
        .map_err(|e| PipelineError::IOError(format!("Failed to open summary file {}: {}", summary_path.display(), e)))?;

    let m8_reader = tokio::io::BufReader::with_capacity(16 * 1024 * 1024, m8_file);
    let summary_reader = tokio::io::BufReader::with_capacity(16 * 1024 * 1024, summary_file);

    let mut m8_lines = m8_reader.lines();
    let mut summary_lines = summary_reader.lines();
    ...
    loop {
        let (m8_res, hit_res) = tokio::join!(
            async {
                let start = Instant::now();
                let item = m8_lines.next_line().await;
                (item, start.elapsed())
            },
            async {
                let start = Instant::now();
                let item = summary_lines.next_line().await;
                (item, start.elapsed())
            }
        );

        let (m8_result, m8_wait) = m8_res;
        let (hit_result, hit_wait) = hit_res;

        if m8_wait > max_m8_wait { max_m8_wait = m8_wait; }
        if hit_wait > max_hit_wait { max_hit_wait = hit_wait; }
        ...
        if m8_wait >= Duration::from_millis(50) || hit_wait >= Duration::from_millis(50) {
            ...
            warn!(
                "[generate_taxon_counts:{}] WAIT: m8={:?} hit={:?} class={} total_pairs={} batches={}",
                count_type, m8_wait, hit_wait, classification, total_pairs, batch_idx
            );
        }

        let m8_line = match m8_result {
            Ok(Some(l)) => l,
            Ok(None) => break,
            Err(e) => return Err(PipelineError::IOError(format!("Error reading m8 file: {}", e))),
        };

        let hit_line = match hit_result {
            Ok(Some(l)) => l,
            Ok(None) => break,
            Err(e) => return Err(PipelineError::IOError(format!("Error reading summary file: {}", e))),
        };

        if !m8_line.is_empty() {
            m8_batch.push(Bytes::from(m8_line.into_bytes()));
            total_pairs += 1;
        }
        if !hit_line.is_empty() {
            hit_batch.push(Bytes::from(hit_line.into_bytes()));
        }
        ...
        if m8_batch.len() >= 8192 && hit_batch.len() >= 8192 {
            ...
            if send_wait > max_send_wait { max_send_wait = send_wait; }
        ...
        if last_log.elapsed() >= Duration::from_secs(10) {
            info!(
                "[generate_taxon_counts:{}] producer progress: pairs={} batches={} elapsed={:?}",
                count_type, total_pairs, batch_idx, producer_start.elapsed()
            );
    ...
    info!(
        ...
        count_type, result.len(), max_m8_wait, max_hit_wait, max_send_wait
    );
...
async fn load_hit_summary_map(path: &PathBuf) -> Result<AHashMap<String, Bytes>, PipelineError> {
    let (rx, producer) = read_file_lines_batched(path.clone(), 8192).await?;
    ...
    while let Some(batch) = stream.next().await {
        for item in batch {
            if let ParseOutput::Bytes(b) = item {
                ...
                if let Some(read_id) = trimmed.split('\t').next() {
                    map.insert(read_id.to_string(), b);
                }
            }
        }
    }
...
/// Merges NT and NR results with NT priority (exact original logic).
/// Now works with materialized files instead of streams.
/// Creates its own TempDir for the merged m8 + hitsummary outputs.
pub async fn compute_merged_taxon_counts(
    config: Arc<RunConfig>,
    nt_m8_path: PathBuf,
    nt_hit_summary_path: PathBuf,
    ...
    // ────────────────────────────────────────────────────────────────
    // Load hit summaries into memory for random access during merge
    // (safe and fast after call_hits_m8 — data volume is now moderate)
    // ────────────────────────────────────────────────────────────────
    let nt_hit_map: AHashMap<String, Bytes> =
        load_hit_summary_map(&nt_hit_summary_path).await?;
    let nr_hit_map: AHashMap<String, Bytes> =
        load_hit_summary_map(&nr_hit_summary_path).await?;

    // ────────────────────────────────────────────────────────────────
    // Write merged m8 + hitsummary with NT priority logic
    // ────────────────────────────────────────────────────────────────
    let mut merged_m8_writer = BufWriter::with_capacity(
        16 * 1024 * 1024,
        TokioFile::create(&merged_m8_path).await?,
    );
    let mut merged_hit_writer = BufWriter::with_capacity(
        16 * 1024 * 1024,
        TokioFile::create(&merged_hitsummary_path).await?,
    );

    // NT pass (higher priority)
    {
        let (m8_rx, m8_producer) = read_file_lines_batched(nt_m8_path.clone(), 8192).await?;
        let mut m8_stream = ReceiverStream::new(m8_rx);

        while let Some(batch) = m8_stream.next().await {
            for item in batch {
                let m8_bytes = match item {
                    ParseOutput::Bytes(b) => b,
                    _ => continue,
                };

                let m8_str = String::from_utf8_lossy(&m8_bytes);
                let trimmed = m8_str.trim_end();
                if trimmed.is_empty() { continue; }

                let m8_fields: Vec<&str> = trimmed.split('\t').collect();
                if m8_fields.is_empty() { continue; }
                let read_id = m8_fields[0];

                if let Some(hit_bytes) = nt_hit_map.get(read_id) {
                    let hit_line = String::from_utf8_lossy(hit_bytes);
                    let hit_trimmed = hit_line.trim_end();
                    if hit_trimmed.is_empty() { continue; }

                    let hit_fields: Vec<&str> = hit_trimmed.split('\t').collect();
                    if hit_fields.len() < 10 { continue; }

                    let nt_contig = hit_fields.get(9).and_then(|s| s.parse::<i32>().ok());
                    let nt_read   = hit_fields.get(3).and_then(|s| s.parse::<i32>().ok());

                    let has_nr_contig = nr_hit_map.get(read_id)
                        .and_then(|b| {
                    ...
                    if nt_contig.is_some() || (!has_nr_contig && nt_read.is_some()) {
                        // Write merged m8
                        merged_m8_writer.write_all(&m8_bytes).await?;
                        merged_m8_writer.write_all(b"\n").await?;

                        // Write merged hitsummary with NT tag
                        let mut hit_with_tag = hit_fields.to_vec();
                        hit_with_tag.push(NT_TAG);
                        let tagged = hit_with_tag.join("\t");
                        merged_hit_writer.write_all(tagged.as_bytes()).await?;
                        merged_hit_writer.write_all(b"\n").await?;
                    }
                }
            }
        }
        let _ = m8_producer.await;
    }

    // NR pass (remaining reads)
    {
        let (m8_rx, m8_producer) = read_file_lines_batched(nr_m8_path.clone(), 8192).await?;
        let mut m8_stream = ReceiverStream::new(m8_rx);
        ...
        while let Some(batch) = m8_stream.next().await {
            for item in batch {
                ...
                // Only include if it was NOT already taken by NT
                if !nt_hit_map.contains_key(read_id) {
                    if let Some(hit_bytes) = nr_hit_map.get(read_id) {
                        merged_m8_writer.write_all(&m8_bytes).await?;
                        merged_m8_writer.write_all(b"\n").await?;

                        let hit_line = String::from_utf8_lossy(hit_bytes);
                        let mut hit_with_tag: Vec<String> = hit_line
                            .trim_end()
                            .split('\t')
                            .map(|s| s.to_string())
                            .collect();
                        hit_with_tag.push(NR_TAG.to_string());
    ...
    merged_m8_writer.flush().await?;
    merged_hit_writer.flush().await?;

    // ────────────────────────────────────────────────────────────────
    // Run generate_taxon_counts on the merged files we just wrote
    // ────────────────────────────────────────────────────────────────
    let merged_taxon_counts = generate_taxon_counts(
        config.clone(),
        merged_m8_path.clone(),
        merged_hitsummary_path.clone(),
    ...
    // ────────────────────────────────────────────────────────────────
    // Merge contig summaries (exact original logic)
    // ────────────────────────────────────────────────────────────────
    let final_contigs = tokio::task::spawn_blocking(move || {
        let mut merged_contigs: HashMap<i32, Vec<ContigSummaryEntry>> = HashMap::new();
...
pub async fn generate_annotated_fasta_stream(
    config: Arc<RunConfig>,
    input_stream: ReceiverStream<ParseOutput>, // contigs.fasta stream
    duplicate_clusters: Arc<DashMap<String, ClusterInfo>>,
    nt_map: AHashMap<String, String>,
    nr_map: AHashMap<String, String>,
    _concurrency: usize,
) -> Result<(
    PathBuf,                    // mapped_contigs_path
    PathBuf,                    // unidentified_contigs_path
    PathBuf,                    // unique_unidentified_path
    TempDir,                    // keep alive until end of pipeline
    Vec<JoinHandle<Result<()>>>,
    Vec<oneshot::Receiver<Result<()>>>,
)> {
    let channel_cap = (config.base_buffer_size / 4).max(10_000);

    // Create temp directory for the three output FASTA files
    let temp_dir = choose_temp_dir(
        2 * 1024 * 1024 * 1024, // generous headroom for annotated contigs
        &config.ram_temp_dir,
        &config.args.nvme_scratch,
        2,
        true,
    ...
    let mapped_path = temp_dir.path().join("mapped_contigs.fasta");
    let unidentified_path = temp_dir.path().join("unidentified_contigs.fasta");
    let unique_unidentified_path = temp_dir.path().join("unique_unidentified_contigs.fasta");

    // Open writers
    let mut mapped_writer = tokio::fs::File::create(&mapped_path).await?;
    let mut unidentified_writer = tokio::fs::File::create(&unidentified_path).await?;
    let mut unique_writer = tokio::fs::File::create(&unique_unidentified_path).await?;
    ...
    let demux_handle = tokio::spawn(async move {
        ...
        while let Some(item) = stream.next().await {
            if let ParseOutput::Bytes(b) = item {
                ...
                // Write the FASTA data to the correct file based on tag
                let writer = match tag {
                    0 => &mut mapped_writer,
                    1 => &mut unidentified_writer,
                    2 => &mut unique_writer,
                    _ => {
                        warn!("Unknown tag in generate_annotated_fasta_stream: {}", tag);
                        continue;
                    }
                };

                // data already contains the full FASTA record(s)
                if let Err(e) = writer.write_all(data).await {
                    warn!("Failed to write annotated FASTA record: {}", e);
                }
            }
        }

        // Flush all writers
        let _ = mapped_writer.flush().await;
        let _ = unidentified_writer.flush().await;
        let _ = unique_writer.flush().await;

        let _ = done_tx.send(Ok(()));
        info!("generate_annotated_fasta_stream completed (wrote to temp files)");
        Ok::<(), anyhow::Error>(())
    });
...
pub async fn summarize_hits(
    ...
    info!(
        "[summarize_hits] starting from file {} (concurrency={})",
        summary_path.display(),
        concurrency
    );

    let (batch_rx, producer_handle) = read_file_lines_batched(
        summary_path.clone(),
        8192,
    ).await?;
    ...
    info!(
        "summarize_hits: processed {} reads, {} unique accessions ({} workers) from {}",
        total_reads_val, final_accession_dict.len(), concurrency, summary_path.display()
    );
...
pub async fn update_read_dict(
    ...
    PathBuf,   // contig2lineage_path
    PathBuf,   // read2blastm8_path
    PathBuf,   // updated_reads_path
    PathBuf,   // added_reads_path
    TempDir,   // keep alive until end of pipeline
)> {
    ...
    // Pass 1: build merged contig2accession map
    ...
    let temp_dir = choose_temp_dir(
        256 * 1024 * 1024, // ~256 MB headroom is plenty for these outputs
        &config.ram_temp_dir,
        &config.args.nvme_scratch,
        2,
        true,
    ...
    let contig2lineage_path = temp_dir.path().join("contig2lineage.tab");
    let read2blastm8_path = temp_dir.path().join("read2blastm8.tab");
    let updated_reads_path = temp_dir.path().join("updated_reads.tab");
    let added_reads_path = temp_dir.path().join("added_reads.tab");

    // Pass 2: parallel fan-out
        ...
        tokio::task::spawn_blocking(move || -> Result<Vec<FanoutBatch>> {
            let batches = contig2reads
                ...
                .map(|(contig_id, read_ids)| {
                    ...
                    let m8_line = m8_row.to_tab_string() + "\n";
    ...
    let mut contig2lineage_file = tokio::fs::File::create(&contig2lineage_path).await?;
    let mut read2blastm8_file = tokio::fs::File::create(&read2blastm8_path).await?;
    let mut updated_file = tokio::fs::File::create(&updated_reads_path).await?;
    let mut added_file = tokio::fs::File::create(&added_reads_path).await?;

    for line in contig2lineage_lines {
        contig2lineage_file.write_all(&line).await?;
    }
    for line in read2blastm8_lines {
        read2blastm8_file.write_all(&line).await?;
    }

    for (read_id, hit) in updated_entries {
        let mut line = hit.to_full_tab_line(&read_id);
        line.push('\n');
        updated_file.write_all(line.as_bytes()).await?;
    }

    for (read_id, hit) in added_entries {
        let mut line = hit.to_full_tab_line(&read_id);
        line.push('\n');
        added_file.write_all(line.as_bytes()).await?;
    }

    contig2lineage_file.flush().await?;
    read2blastm8_file.flush().await?;
    updated_file.flush().await?;
    added_file.flush().await?;

    info!(
        "[update_read_dict:{}] wrote outputs to temp dir {}",
        db_type,
        temp_dir.path().display()
    );
...
/// Collects a file into a map keyed by the first tab-delimited field.
/// Preserves first-seen key order.
/// Collects lines from a file (via batched reader) into a map keyed by the first tab field.
/// Preserves first-seen insertion order.
async fn collect_line_map_from_file(
    path: PathBuf,
    batch_size: usize,
) -> Result<(AHashMap<String, Bytes>, Vec<String>)> {
    let (rx, _) = read_file_lines_batched(path, batch_size).await?;
    let mut stream = ReceiverStream::new(rx);
...
/// Collects lines from a file into keyed rows while preserving first-seen key order.
async fn collect_keyed_rows_from_file(
    path: PathBuf,
    batch_size: usize,
) -> Result<(Vec<(String, Bytes)>, Vec<String>)> {
    let (rx, _) = read_file_lines_batched(path, batch_size).await?;
...
pub async fn generate_m8_and_hit_summary(
    config: Arc<RunConfig>,
    updated_reads_path: PathBuf,
    added_reads_path: PathBuf,
    blast_hits_path: PathBuf,
    original_hit_summary_path: PathBuf,
    original_deduped_m8_path: PathBuf,
) -> Result<(
    PathBuf,   // refined_m8_path
    PathBuf,   // refined_hit_summary_path
    TempDir,
)> {
    // Read the three materialized files from update_read_dict
    let updated_fut    = collect_line_map_from_file(updated_reads_path, 8192);
    let added_fut      = collect_line_map_from_file(added_reads_path, 8192);
    let blast_hits_fut = collect_line_map_from_file(blast_hits_path, 8192);

    // These two are now also files (will come from blast_contigs signature)
    let hit_rows_fut = collect_keyed_rows_from_file(original_hit_summary_path, 8192);
    let m8_rows_fut  = collect_keyed_rows_from_file(original_deduped_m8_path, 8192);
        ...
        rayon::join(
            || {
                original_m8_rows
                    .par_iter()
                    .map(|(qseqid, row)| {
                        blast_hits.get(qseqid).cloned().unwrap_or_else(|| row.clone())
                    })
                    .collect::<Vec<Bytes>>()
            },
            || {
    ...
    // Create temp directory for refined outputs
    let temp_dir = choose_temp_dir(
        512 * 1024 * 1024, // generous headroom for refined m8 + hitsummary
        &config.ram_temp_dir,
        &config.args.nvme_scratch,
        2,
        true,
    ...
    let refined_m8_path = temp_dir.path().join("refined.m8");
    let refined_hit_summary_path = temp_dir.path().join("refined_hit_summary.tab");

    // Write refined M8
    let mut m8_file = tokio::fs::File::create(&refined_m8_path).await?;
    for row in rewritten_m8 {
        m8_file.write_all(&row).await?;
    }

    // Append new blast rows that didn't exist in the original deduped m8
    ...
    // Write refined hit summary
    let mut hit_file = tokio::fs::File::create(&refined_hit_summary_path).await?;
    for row in rewritten_hit_summary {
        hit_file.write_all(&row).await?;
    }

    // Append new added reads
    ...
    hit_file.flush().await?;

    info!(
        "[generate_m8_and_hit_summary] wrote refined outputs to {}",
        temp_dir.path().display()
    );

    Ok((refined_m8_path, refined_hit_summary_path, temp_dir))
}
...
pub async fn blast_contigs(
    config: Arc<RunConfig>,
    db_type: &'static str,
    deduped_m8_path: PathBuf,      // ← was stream
    hit_summary_path: PathBuf,     // ← was stream
    read_dict: Arc<Mutex<AHashMap<String, Arc<ReadHit>>>>,
    accession_map: Arc<AHashMap<String, AccessionHit>>,
    ...
    PathBuf,                           // refined_m8_path     ← changed
    PathBuf,                           // refined_hit_summary_path ← changed
    mpsc::Receiver<ParseOutput>,       // top_for_caller_rx   (still streaming)
    Vec<JoinHandle<Result<()>>>,
    Vec<oneshot::Receiver<Result<()>>>,
    Vec<NamedTempFile>,
    Vec<TempDir>,
    ...
    if contig_size < MIN_REF_FASTA_SIZE || ref_size < MIN_ASSEMBLED_CONTIG_SIZE {
        ...
        return Ok((
            ...
            taxon_counts,
            vec![],
            refined_m8_path,           // ← now a PathBuf (empty file)
            refined_hit_summary_path,  // ← now a PathBuf (empty file)
            top_rx,                    // ← still a dummy receiver for top hits
            cleanup_tasks,
            cleanup_receivers,
            temp_files,
            temp_dirs,
    ...
    let update_result = update_read_dict(
        config.clone(),
        read2contig.clone(),
    ...
    temp_dirs.push(update_temp_dir);

    info!(
        "[blast_contigs:{}] update_read_dict finished — wrote outputs to temp files",
        db_type
    );

    let contig2lineage_task = tokio::spawn({
        let contig2lineage_path = contig2lineage_path.clone();
        let start = Instant::now();
        info!("[blast_contigs:{}] contig2lineage collector started", db_type);

        async move {
            let file = TokioFile::open(&contig2lineage_path).await?;
    ...
    // Await it early because taxon counting depends on its output files
    let (refined_m8_path, refined_hit_summary_path, generate_temp_dir) =
        generate_m8_handle.await??;

    temp_dirs.push(generate_temp_dir);
    ...
    let counts_handle = tokio::spawn({
        ...
        let fut = generate_taxon_count_json_from_m8(
            refined_m8_path.clone(),           // ← clone here
            refined_hit_summary_path.clone(),  // ← clone here
            db_type,
            lineage_map.clone(),
            should_keep_filter.clone(),
            duplicate_clusters.clone(),
    ...
    Ok((
        ...
        refined_counts,
        contig_summary,
        refined_m8_path,           // ← now a PathBuf
        refined_hit_summary_path,  // ← now a PathBuf
        top_for_caller_rx,
        cleanup_tasks,
        cleanup_receivers,
        temp_files,
...
// Clone paths **before** moving them into async copy tasks
    ...
    // right here we now have an m8 and hit summary file materilzed to fast temp
    // everytuing that needed nr_call_stream can use nr_temp_m8_path
    // everything that needed nr_call_summary_stream can use nr_temp_summary_path

    let nr_hit_summary_handle = tokio::spawn({
        let config = config.clone();
    ...
    let nr_counts_task = tokio::spawn({
        ...
        let should_keep_filter = should_keep_filter.clone();
        let duplicate_clusters = duplicate_clusters.clone();
        let nr_temp_m8_path = nr_temp_m8_path.clone();           // from call_hits_m8
        let nr_temp_summary_path = nr_temp_summary_path.clone(); // from call_hits_m8
        async move {
            let start = Instant::now();
            info!("[run] generate_taxon_counts(NR) started");
            let res = generate_taxon_counts(
                config,
                nr_temp_m8_path,        // ← now a file path
                nr_temp_summary_path,   // ← now a file path
                duplicate_clusters,
                should_keep_filter,
                "NR".to_string(),
                None,
            ).await;
            info!("[run] generate_taxon_counts(NR) finished after {:?}", start.elapsed());
            res
        }
    });
    ...
    let (
        initial_mapped_path,
        initial_unidentified_path,
        _initial_unique_unidentified_path,
        initial_annot_temp_dir,
        mut initial_annot_tasks,
        mut initial_annot_rxs,
    ...
    final_temp_dirs.push(initial_annot_temp_dir);
    cleanup_tasks.append(&mut initial_annot_tasks);
    cleanup_receivers.append(&mut initial_annot_rxs);

    let annotated_final_path = assembly_out_dir.join("annotated_merged.fa");
    let unidentified_final_path = assembly_out_dir.join("unidentified.fa");
    let unique_unidentified_final_path = assembly_out_dir.join("unique_unidentified.fa");

    tokio::fs::copy(&initial_mapped_path, &annotated_final_path).await?;
    tokio::fs::copy(&initial_unidentified_path, &unidentified_final_path).await?;
    tokio::fs::copy(&_initial_unique_unidentified_path, &unique_unidentified_final_path).await?;
    ...
    //NB: anything downastream needing "Refined..." stremas shouold now use the nt or nr refined FILES (m8 or hit summary)



    let (nt_res, nr_res) = tokio::join!(nt_handle, nr_handle);
    ...
    let (
        nt_read_dict,                    // AHashMap<String, Arc<ReadHit>>
        nt_refined_counts,               // Vec<TaxonCount>
        nt_contig_summary,               // Vec<ContigSummaryEntry>
        nt_refined_m8_path,              // PathBuf  ← refined m8 file
        nt_refined_hit_summary_path,     // PathBuf  ← refined hit summary file
        nt_refined_top_stream,           // mpsc::Receiver<ParseOutput>  (from top-hits)
        nt_cleanup_tasks,                // Vec<JoinHandle<Result<()>>>
        nt_cleanup_receivers,            // Vec<oneshot::Receiver<Result<()>>>
        nt_temp_files,                   // Vec<NamedTempFile>
        nt_temp_dirs,                    // Vec<TempDir>
    ) = nt_res;
    cleanup_tasks.extend(nt_cleanup_tasks);
    cleanup_receivers.extend(nt_cleanup_receivers);
    temp_files.extend(nt_temp_files);
    final_temp_dirs.extend(nt_temp_dirs);

    let (
        nr_read_dict,                    // AHashMap<String, Arc<ReadHit>>
        nr_refined_counts,               // Vec<TaxonCount>
        nr_contig_summary,               // Vec<ContigSummaryEntry>
        nr_refined_m8_path,              // PathBuf  ← refined m8 file
        nr_refined_hit_summary_path,     // PathBuf  ← refined hit summary file
        nr_refined_top_stream,           // mpsc::Receiver<ParseOutput>  (from top-hits)
        nr_cleanup_tasks,                // Vec<JoinHandle<Result<()>>>
        nr_cleanup_receivers,            // Vec<oneshot::Receiver<Result<()>>>
        nr_temp_files,                   // Vec<NamedTempFile>
        nr_temp_dirs,                    // Vec<TempDir>
    ) = nr_res;
    cleanup_tasks.extend(nr_cleanup_tasks);
    cleanup_receivers.extend(nr_cleanup_receivers);
    temp_files.extend(nr_temp_files);
    final_temp_dirs.extend(nr_temp_dirs);


    let merged_cleanup_tasks = compute_merged_taxon_counts(
        config.clone(),
        nt_refined_m8_path.clone(),                    // refined m8 from blast_contigs
        nt_refined_hit_summary_path.clone(),           // refined hitsummary from blast_contigs
        nt_contig_summary,
        nr_refined_m8_path.clone(),                    // refined m8 from blast_contigs
        nr_refined_hit_summary_path.clone(),           // refined hitsummary from blast_contigs
        nr_contig_summary,
        should_keep_filter.clone(),
        duplicate_clusters.clone(),
    ...
    let (nt_refined_map, nr_refined_map) = tokio::try_join!(
        collect_hit_summary_to_accession_map_concurrent(
            config.clone(),
            nt_refined_hit_summary_path.clone(),   // refined
        ),
        collect_hit_summary_to_accession_map_concurrent(
            config.clone(),
            nr_refined_hit_summary_path.clone(),   // refined
        ),
    )?;
    ...
    let (
        ...
        _refined_unique_unidentified_path,
        refined_annot_temp_dir,
        mut refined_annot_tasks,
        mut refined_annot_rxs,
    ) = generate_annotated_fasta_stream(
        config.clone(),
        contigs_stream,
        duplicate_clusters.clone(),
    ...
    final_temp_dirs.push(refined_annot_temp_dir);
    cleanup_tasks.append(&mut refined_annot_tasks);
    cleanup_receivers.append(&mut refined_annot_rxs);
